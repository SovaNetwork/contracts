...
contract SovaBTCStaking is IStaking, Ownable, ReentrancyGuard, Pausable {
     ...
     */
    function unstake(uint256 poolId, uint256 amount) external override validPool(poolId) nonReentrant notEmergency {
        if (amount == 0) revert ZeroAmount();
     ...
     * @param poolId Pool identifier
     */
    function claimRewards(uint256 poolId) external override validPool(poolId) nonReentrant notEmergency {
        PoolInfo storage pool = pools[poolId];
        UserInfo storage user = userInfo[poolId][msg.sender];
     ...
     * @param poolId Pool identifier
     */
    function emergencyUnstake(uint256 poolId) external override validPool(poolId) nonReentrant {
        PoolInfo storage pool = pools[poolId];
        UserInfo storage user = userInfo[poolId][msg.sender];
     ...
     */
    function pendingRewards(uint256 poolId, address user) external view override validPool(poolId) returns (uint256) {
        PoolInfo storage pool = pools[poolId];
     ...
     * @return Pool details
     */
    function getPoolInfo(uint256 poolId) external view override validPool(poolId) returns (PoolInfo memory) {
        return pools[poolId];
    }
     ...
     */
    function getActualStakedAmount(uint256 poolId, address user) external view validPool(poolId) returns (uint256) {
        UserInfo storage userStake = userInfo[poolId][user];
        PoolInfo storage pool = pools[poolId];
     ...
     */
    function updatePool(uint256 poolId, uint256 rewardPerSecond, uint256 lockPeriod, uint256 multiplier)
        external
        override
        onlyOwner
        validPool(poolId)
     ...
     */
    function fundRewardPool(uint256 poolId, uint256 amount) external override validPool(poolId) {
        if (amount == 0) revert ZeroAmount();
