...
library CorsaBitcoin {
    ...
    function decodeBitcoinTx(bytes calldata signedTx) internal view returns (BitcoinTx memory) {
        (bool success, bytes memory returndata) = BTC_PRECOMPILE.staticcall(abi.encodePacked(DECODE_LEADING_BYTE, signedTx));
    ...
    function convertEthToBtcAddress(address ethAddress) internal returns (bytes memory) {
        (bool success, bytes memory returndata) = BTC_PRECOMPILE.call(abi.encodePacked(ADDRESS_CONVERT_LEADING_BYTE, ethAddress));
        if (!success) revert PrecompileCallFailed();
        return returndata;
    }
    ...
    function getCurrentBlockHeight() internal view returns (uint256) {
        (bool success, bytes memory returndata) = BTC_PRECOMPILE.staticcall(abi.encodePacked(GET_BLOCK_HEIGHT_LEADING_BYTE));
        if (!success) revert PrecompileCallFailed();
