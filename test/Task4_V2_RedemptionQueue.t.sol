...
contract Task4V2RedemptionQueueTest is Test {
    ...
    function test_QueueRedemption_InsufficientReserve() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                RedemptionQueue.InsufficientReserve.selector,
                excessiveAmount,
                wbtc.balanceOf(address(redemptionQueue))
            )
        );
    ...
    function test_QueueRedemption_ExistingPendingRedemption() public {
        ...
        sovaBTC.approve(address(redemptionQueue), redeemAmount);
        vm.expectRevert(
            abi.encodeWithSelector(RedemptionQueue.ExistingPendingRedemption.selector, user1)
        );
        redemptionQueue.redeem(address(wbtc), redeemAmount);
    ...
    function test_FulfillRedemption_SuccessAfterDelay() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                RedemptionQueue.RedemptionNotReady.selector,
                block.timestamp,
                block.timestamp + DEFAULT_REDEMPTION_DELAY
            )
        );
    ...
    function test_FulfillRedemption_OnlyAuthorizedCustodian() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(RedemptionQueue.UnauthorizedCustodian.selector, nonCustodian)
        );
        redemptionQueue.fulfillRedemption(user1);
    ...
    function test_FulfillRedemption_AlreadyFulfilled() public {
        ...
        vm.prank(custodian);
        vm.expectRevert(
            abi.encodeWithSelector(RedemptionQueue.RedemptionAlreadyFulfilled.selector, user1)
        );
        redemptionQueue.fulfillRedemption(user1);
    }
    ...
    function test_SetRedemptionDelay_InvalidDelay() public {
        ...
        // Too short
        vm.expectRevert(
            abi.encodeWithSelector(RedemptionQueue.InvalidRedemptionDelay.selector, 30 minutes)
        );
        ...
        // Too long
        vm.expectRevert(
            abi.encodeWithSelector(RedemptionQueue.InvalidRedemptionDelay.selector, 60 days)
        );
