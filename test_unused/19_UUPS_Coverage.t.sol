...
contract TestUpgradeableSovaBTC is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address _owner, uint64 _minDepositAmount, uint64 _maxDepositAmount, uint64 _maxGasLimitAmount)
        external
        initializer
    {
    ...
    function withdraw(uint64 amount, uint64 gasLimit, string calldata destination) external {
        pendingWithdrawals[msg.sender] =
            PendingOperation({amount: amount, gasLimit: gasLimit, destination: destination, timestamp: block.timestamp});
    }
...
contract UUPSCoverageTest is Test {
    ...
    function test_AllContractFunctions() public {
        ...
        (uint64 amount, uint64 gasLimit, string memory destination, uint256 timestamp) =
            contract_.pendingWithdrawals(user);
    ...
    function test_AuthorizeUpgradeFunction() public {
        ...
        bytes memory initData =
            abi.encodeWithSelector(TestUpgradeableSovaBTC.initialize.selector, owner, 1e6, 1e10, 1e6);
    ...
    function test_EdgeCasesAndBoundaryValues() public {
        ...
        contract_.withdraw(
            type(uint64).max, type(uint64).max, "very_long_destination_address_that_tests_string_handling"
        );
        vm.stopPrank();
    }
    ...
    function test_TestUpgradeableWithoutConstructorFunctions() public {
        ...
        bytes memory initData =
            abi.encodeWithSelector(TestUpgradeableWithoutConstructor.initialize.selector, owner, 1e6, 1e10, 1e6);
    ...
    function test_FinalCoverageEnsurance() public {
        ...
        bytes memory initData1 =
            abi.encodeWithSelector(TestUpgradeableSovaBTC.initialize.selector, makeAddr("proxy_owner1"), 0, 0, 0);

        bytes memory initData2 =
            abi.encodeWithSelector(TestUpgradeableSovaBTC.initialize.selector, makeAddr("proxy_owner2"), 1, 1, 1);
