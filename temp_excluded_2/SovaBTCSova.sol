...
contract SovaBTCSova is SovaBTCOFT {
     ...
     */
    constructor(address _endpoint, address _delegate) SovaBTCOFT("Sova Bitcoin", "sovaBTC", _endpoint, _delegate) {
        ...
        whenNotPaused
    {
        ...
        pendingWithdrawals[msg.sender] =
            PendingWithdrawal({amount: totalRequired, timestamp: block.timestamp, exists: true});
    ...
    function getPendingWithdrawal(address user)
        external
        view
        returns (uint256 amount, uint256 timestamp, bool exists)
    {
        PendingWithdrawal memory pending = pendingWithdrawals[user];
